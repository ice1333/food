...
public class ShopMainController {
	...
	public String insert(RestaurantVo vo, HttpServletRequest req, MultipartFile file,MultipartFile file1,MultipartFile file2,MultipartFile file3, HttpSession sess) {
		...
		public String update(RestaurantVo vo, HttpServletRequest req, MultipartFile file,MultipartFile file1,MultipartFile file2,MultipartFile file3, HttpSession sess) {
			...
			if (file != null && !file.isEmpty() || file1 != null && !file1.isEmpty() || file2 != null && !file2.isEmpty() || file3 != null && !file3.isEmpty()) { // 사용자가 파일을 첨부했다면 
				try {
					String path = req.getRealPath("/upload/");
					String filename = file.getOriginalFilename(); // 사용자가 업로드한 원본 파일
					String filename1 = file1.getOriginalFilename(); // 사용자가 업로드한 원본 파일
			...
			if(r > 0) {
			req.setAttribute("msg", "정상적으로 등록되었습니다");
			req.setAttribute("url", "shop/shopmain.do?r_no="+vo.getR_no());
			} else {
				req.setAttribute("msg", "등록 오류 ");
				req.setAttribute("url", "shop/edit.do?r_no="+vo.getR_no());
			}
