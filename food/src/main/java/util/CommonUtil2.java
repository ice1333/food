package util;

public class CommonUtil2 {
	public static String getPageArea(String url, int curPage, int totPage,int pageRange,String visit) {
		// 페이지범위
		int startPage =(curPage-1)/pageRange*pageRange+1;
		int endPage = startPage+pageRange -1; // 종료페이지
		if(endPage>totPage) endPage = totPage;
				
		String ret = "";
		ret+=" <div class=\"pagenate clear\">\n"
				+ "                        <ul class='paging'>\n";
		if(startPage > pageRange) {
		ret += "                        	<li><a href=\""+url+"?page="+(startPage-1)+"\"> < </a>\n";
		}
		if(visit == "" || visit== null) {
			for(int rp = startPage;rp <= endPage;  rp++) {
				ret+=		 "                            <li><a href='javascript:location.href=\""+url+"?page="+rp+"\";'";
				if(rp==curPage) ret+= "class='current'";
				ret += ">"+rp+"</a></li>\r\n";
			}
			
		}else {
			for(int rp = startPage;rp <= endPage;  rp++) {
				ret+=		 "                            <li><a href='javascript:location.href=\""+url+"?searchType=&searchWord=&visit="+visit+"&page="+rp+"\";'";
				if(rp==curPage) ret+= "class='current'";
				ret += ">"+rp+"</a></li>\r\n";
			}
		}
		if(totPage>endPage) {
			ret += "                        	<li><a href=\""+url+"?page="+(endPage+1)+"\"> > </a>\r\n";
		}
		ret += "                        </ul> \n"
				+"                    </div>";
		return ret;
	}
}
